...
def call_judge_agent_premium(
    ...
    prompt = (
        ...
        f"de cambio, tasas de banco central, rentabilidades históricas de mercado). Rechaza (✗) "
        f"cualquiera de estas si:\n"
        f"  a) aparece en el subject o el preheader (las cifras de mercado solo pueden ir en el cuerpo)\n"
        f"  b) el número no coincide con ninguna de las CIFRAS DE MERCADO VERIFICADAS de abajo\n"
        f"  c) no tiene un período explícito pegado en la misma frase (ej. 'esta semana', 'en julio', "
        f"'acumulado del año') — una cifra sin período es ambigua y se rechaza aunque el número sea correcto\n"
        f"  d) hay más de una cifra de mercado distinta en el mismo nodo (máximo 1 por nodo)\n\n"
        f"PASO 3 — Decide:\n"
